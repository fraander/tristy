...
class ViewModel: ObservableObject {
    ...
    @Published var items: [TristyList]
    ...
    init() {
        do {
            ...
            items = try JSONDecoder().decode([TristyList].self, from: data)
        } catch {
            // Loading failed, so start with an empty array.
            items = []
        }

        // Wait 0.5 seconds after `items` has changed before calling `save()`, to
    ...
    func delete(_ selected: Set<TristyList>) {
